                                    ...
                                    <div class="top-content">
                                        <div class="preview">
                                            <img data-rid="${item.id}" data-index="0" class="scroll-loading"
                                                 src="${item.sellerLogo}" data-src="${item.sellerLogo}"
                                                 data-max-width="208" data-max-height="156">
                                            <div class="rest-tags"></div>
                                        </div>
                                        <div class="content">
                                            <div class="name"><span title="${item.sellerName}">${item.sellerName}</span>
                                            </div>
                                            <div class="rank clearfix"><span class="star-ranking fl"> <span
                                                    class="star-score" style="width: 68px"></span> </span> <span
                                                    class="score-num fl">${item.scoreNum}</span>
                                            </div>
                                            <div class="price"><span class="start-price">起送:￥${item.startPrice}</span>
                                                <span class="send-price"> 配送费￥${item.sendPrice} </span>
                                                <span class="send-time"><i class="icon i-poi-timer"></i>${item.sendTime}</span>
                                            </div>
                                        </div>
                                        <div class="clear"></div>
                                    </div>
...
<%--var myid = document.getElementById('myyid').innerText;--%>
<%--<c:forEach var="item" items="${seller}" varStatus="se">--%>
...
<%--t1+='<a class="rest-atag" href="${pageContext.request.contextPath}/order/main?sellerId='+'${item.sellerId}&id='+myid+'" target="_blank">';--%>
